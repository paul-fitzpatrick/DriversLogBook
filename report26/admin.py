...
from .forms import UpdateForm26
import json
import requests
...
@admin.register(report_26)
class report_26Admin(admin.ModelAdmin):
    ...
    actions = ['download_information26', 'send_email_action', 'pdf_download']  # Registering the custom actions

    def approve_report(self, request, queryset):
        queryset.update(report_approved=True)

    def download_information26(self, request, queryset):
    ...
    download_information26.short_description = 'Download Logbook Report CSV'  # Action description for the admin interface
