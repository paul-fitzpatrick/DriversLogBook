...
from .forms import UpdateFormmk4
import json
import requests
...
@admin.register(report_mk4)
class report_mk4Admin(admin.ModelAdmin):
    ...
    actions = ['download_informationmk4', 'send_email_action', 'pdf_download']  # Registering the custom actions

    def approve_report(self, request, queryset):
        queryset.update(report_approved=True)

    def download_informationmk4(self, request, queryset):
    ...
    download_informationmk4.short_description = 'Download Logbook Report CSV'  # Action description for the admin interface
