...
from .forms import UpdateForm85
import json
import requests
...
@admin.register(report_85)
class report_85Admin(admin.ModelAdmin):
    ...
    actions = ['download_information85', 'send_email_action', 'pdf_download']  # Registering the custom actions

    def approve_report(self, request, queryset):
        queryset.update(report_approved=True)

    def download_information85(self, request, queryset):
    ...
    download_information85.short_description = 'Download Logbook Report CSV'  # Action description for the admin interface
