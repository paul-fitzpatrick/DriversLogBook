...
from .forms import UpdateForm81
import json
import requests
...
@admin.register(report_81)
class report_81Admin(admin.ModelAdmin):
    ...
    actions = ['download_information81', 'send_email_action', 'pdf_download']  # Registering the custom actions

    def approve_report(self, request, queryset):
        queryset.update(report_approved=True)

    def download_information81(self, request, queryset):
    ...
    download_information81.short_description = 'Download Logbook Report CSV'  # Action description for the admin interface
