...
from .forms import UpdateForm201
import json
import requests
...
@admin.register(report_201)
class report_201Admin(admin.ModelAdmin):
    ...
    actions = ['download_information201', 'send_email_action', 'pdf_download']  # Registering the custom actions

    def approve_report(self, request, queryset):
        queryset.update(report_approved=True)

    def download_information201(self, request, queryset):
    ...
    download_information201.short_description = 'Download Logbook Report CSV'  # Action description for the admin interface
