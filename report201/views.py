...
from .models import report_201
from django.contrib import messages
from .forms import ReportForm201, UpdateForm201
...
def index201(request):
    """ A view to return the index201 page """

    return render(request, 'index201.html')

# render report detail page
def report_detail201(request, report_201_id):
    """ A view to return the report details page """
    report201 = get_object_or_404(report_201, pk=report_201_id)

    # Get the previous and next items
    previous_item = report_201.objects.filter(id__lt=report_201_id).order_by('-id').first()
    next_item = report_201.objects.filter(id__gt=report_201_id).order_by('id').first()
    ...
    return render(request, 'report_detail201.html', context)


# add report view
def add_report201(request):
    if request.method == "POST":
        form = ReportForm201(request.POST, request=request)
        if form.is_valid():
            form.instance.customer_email = request.user.email
            form.instance.name = request.user.username
            report201 = form.save(commit=False)
            report201.save()
            ...
            return redirect('open_reports201.html')  # Redirect to the desired page after adding the report
    else:
        form = ReportForm201(request=request)
    
    context = {
        'form': form,
        'report201_added': True,
    }
    
    return render(request, 'add_report201.html', context)


# Search box and open reports view
def reports_list201(request):
    """ A View to return all read/work order reports and search queries """

    reports201 = report_201.objects.all() 
    query = None
    categories = None
    sort = None
    direction = None
    ...
    if request.GET:
        if 'sort' in request.GET:
            ...
            sort = sortkey    
            reports201 = reports201.order_by(sortkey)
        ...
        if 'q' in request.GET:
            ...
            reports201 = reports201.filter(queries)
    current_sorting = f'{sort}_{direction}'

    context = {
        'reports201': reports201,
        'search_term': query,
        
        'current_sorting': current_sorting,
    }
    return render(request, 'open_reports201.html', context)
...
#     reports = report_201.objects.filter(driver_email=request.user.email)
...
def unit_history201(request):
    ...
    return render(request, 'unit_history201.html', context)


# def my_messages201(request):
#     messages = report_201.objects.filter(driver_name=request.user.username).order_by('-date_of_fault')
#     return render(request, 'my_messages.html', {'messages': messages})


def update_driver_view201(request):
    return render(request, 'update_driver201.html')


def update_drivermail201(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            update.save()
            return redirect('open_reports201.html')  # Redirect to the desired URL after form submission

    context = {
        'form': form,
    }
    
    return render(request, 'update_driver201.html', context)
    

# hidden form for emailing drivers of update
def update_report_201(request, report_201_id):
    report_201 = get_object_or_404(report_201, pk=report_201_id)

    if request.method == 'POST':
        # Handle the form submission
        form = UpdateForm(request.POST, instance=report_201)
        if form.is_valid():
            form.save()
            # Send email code here
            return redirect('admin:report_report_201_changelist')  # Redirect to the changelist view
    else:
        # Display the form
        form = UpdateForm(instance=report_201)

    return render(request, 'update_driver201.html', {'form': form})