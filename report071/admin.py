...
from .forms import UpdateForm071
import json
import requests
...
@admin.register(report_071)
class report_071Admin(admin.ModelAdmin):
    ...
    actions = ['download_information071', 'send_email_action', 'pdf_download']  # Registering the custom actions

    def approve_report(self, request, queryset):
        queryset.update(report_approved=True)

    def download_information071(self, request, queryset):
    ...
    download_information071.short_description = 'Download Logbook Report CSV'  # Action description for the admin interface
