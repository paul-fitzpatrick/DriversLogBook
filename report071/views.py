...
from .models import report_071
from django.contrib import messages
from .forms import ReportForm071, UpdateForm071
...
def index071(request):
    """ A view to return the index071 page """

    return render(request, 'index071.html')

# render report detail page
def report_detail071(request, report_071_id):
    """ A view to return the report details page """
    report071 = get_object_or_404(report_071, pk=report_071_id)

    # Get the previous and next items
    previous_item = report_071.objects.filter(id__lt=report_071_id).order_by('-id').first()
    next_item = report_071.objects.filter(id__gt=report_071_id).order_by('id').first()
    ...
    return render(request, 'report_detail071.html', context)


# add report view
def add_report071(request):
    if request.method == "POST":
        form = ReportForm071(request.POST, request=request)
        if form.is_valid():
            form.instance.customer_email = request.user.email
            form.instance.name = request.user.username
            report071 = form.save(commit=False)
            report071.save()
            ...
            return redirect('open_reports071.html')  # Redirect to the desired page after adding the report
    else:
        form = ReportForm071(request=request)
    
    context = {
        'form': form,
        'report071_added': True,
    }
    
    return render(request, 'add_report071.html', context)


# Search box and open reports view
def reports_list071(request):
    """ A View to return all read/work order reports and search queries """

    reports071 = report_071.objects.all() 
    query = None
    categories = None
    sort = None
    direction = None
    ...
    if request.GET:
        if 'sort' in request.GET:
            ...
            sort = sortkey    
            reports071 = reports071.order_by(sortkey)
        ...
        if 'q' in request.GET:
            ...
            reports071 = reports071.filter(queries)
    current_sorting = f'{sort}_{direction}'

    context = {
        'reports071': reports071,
        'search_term': query,
        
        'current_sorting': current_sorting,
    }
    return render(request, 'open_reports071.html', context)
...
def my_reports(request):
    ...
    reports = report_071.objects.filter(driver_email=request.user.email)
...
def unit_history071(request):
    ...
    return render(request, 'unit_history071.html', context)


# def my_messages071(request):
#     messages = report_071.objects.filter(driver_name=request.user.username).order_by('-date_of_fault')
#     return render(request, 'my_messages.html', {'messages': messages})


def update_driver_view071(request):
    return render(request, 'update_driver071.html')


def update_drivermail071(request):
    ...
    if request.method == "POST":
        if form.is_valid():
            ...
            update.save()
            return redirect('open_reports071.html')  # Redirect to the desired URL after form submission

    context = {
        'form': form,
    }
    
    return render(request, 'update_driver071.html', context)
    

# hidden form for emailing drivers of update
def update_report_071(request, report_071_id):
    report_071 = get_object_or_404(report_071, pk=report_071_id)

    if request.method == 'POST':
        # Handle the form submission
        form = UpdateForm(request.POST, instance=report_071)
        if form.is_valid():
            form.save()
            # Send email code here
            return redirect('admin:report_report_071_changelist')  # Redirect to the changelist view
    else:
        # Display the form
        form = UpdateForm(instance=report_071)

    return render(request, 'update_driver071.html', {'form': form})